#!/usr/bin/env python3
# ac_kondos_diffusion_bee_fork.py
# AC Kondo's Diffusion Bee Fork — single-file desktop app (no extra apps).
...
APP_TITLE = "AC Kondo's Diffusion Bee Fork"
APP_NAME = "AC Kondo's Diffusion Bee Fork"
TITLEBAR_H = 55
SIDEBAR_W = 200
CARD_W, CARD_H = 280, 230
# System UI font stack (Diffusion Bee: -apple-system / BlinkMacSystemFont)
import sys as _sys
UI_FONT = "Helvetica Neue" if _sys.platform == "darwin" else "Segoe UI"
UI_FONT_TEXT = UI_FONT

# Theme palettes (Diffusion Bee light + dark counterpart)
LIGHT_THEME = {
    "BG": "#F2F2F2",
    "TOP": "#FFFFFF",
    "PANEL": "#F4F5F5",
    ...
    "SIDEBAR_SEL": "#E6E6E6",
    "INPUT_BG": "#F1F4FA",
    "CONTENT": "#F2F2F2",  # Diffusion Bee .animatable_content_box
    "ENGINE": "#E8E8E8",  # Diffusion Bee .image_area (~rgba(0,0,0,0.05))
...
DARK_THEME = {
    ...
    "SIDEBAR_SEL": "#1a1b1f",
    "INPUT_BG": "#111318",
    "CONTENT": "#1c1c1c",  # Diffusion Bee dark .animatable_content_box
    "ENGINE": "#302e2f",  # Diffusion Bee dark .image_area
...
# All actionable buttons use AC Kondo's black-and-blue button treatment.
BUTTON_BG = "#000000"
BUTTON_BG_ACTIVE = "#111111"
BUTTON_TEXT = "#0A84FF"
BUTTON_TEXT_ACTIVE = "#3E7BFA"
...
HOME = Path.home() / ".ac_kondos_diffusion_bee_fork"
LLM_DIR = HOME / "llms"
...
OUTPUT_DIR = os_pictures_dir() / "AC Kondo's Diffusion Bee Fork"
...
def ac_kondos_diffusion_bee_image_stem(prompt: str) -> str:
    """Build a compact image filename from the prompt plus a random integer."""
...
def ensure_png_bytes(data: bytes) -> bytes:
    """JPEG/WebP off. Convert any still image to PNG for this fork."""
    if not data:
...
def apply_negative_prompt(prompt: str, negative: str) -> str:
    """Fork-style negative prompt (FLUX has no native field)."""
...
# AC Kondo's Diffusion Bee Fork applet defaults per tool mode
...
def flux_models_by_family(kind: str = "image", img2img_only: bool = False) -> list[str]:
    """Grouped picker for all FLUX photo/video versions by family."""
    out: list[str] = []
...
# Map the fork's FLUX labels → Pollinations model ids
FREE_FLUX_MODEL_MAP = {
...
# Pre-baked Hugging Face LLMs for FLUX prompt vibes (download on demand into the app data folder)
...
# AC Kondo's Diffusion Bee Fork categories + card copy
HOME_SECTIONS = [
    (
        "All AI Tools",
        [
...
# AC Kondo's Diffusion Bee Fork sidebar
...
def _http_json(
    ...
    data = None
    hdrs = {"Accept": "application/json", "User-Agent": "ACKondosDiffusionBeeFork/1.0"}
    if headers:
        hdrs.update(headers)
    if body is not None:
...
def _download_bytes(url: str, timeout: float = 120.0) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "ACKondosDiffusionBeeFork/1.0"})
    with urllib.request.urlopen(req, timeout=timeout) as resp:
        return resp.read()
...
def _download_file(
    ...
    hdrs = {"User-Agent": "ACKondosDiffusionBeeFork/1.0"}
    if headers:
        hdrs.update(headers)
    req = urllib.request.Request(url, headers=hdrs, method="GET")
    with urllib.request.urlopen(req, timeout=timeout) as resp:
...
class HuggingFaceLLMStore:
    """Pre-baked + vibe-added HF GGUF catalog stored in the fork's app data folder."""
    ...
    def _auth_headers(self) -> dict:
        h = {"User-Agent": "ACKondosDiffusionBeeFork/1.0"}
        if self.hf_token:
...
class AppConfig:
    ...
    output_format: str = "png"  # JPEG off — the fork always writes PNG
    ...
    def load(cls) -> "AppConfig":
        ...
        # Auto-export to the operating system's Pictures folder.
        try:
...
class FluxBackend:
    ...
    def _generate_free(
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": "ACKondosDiffusionBeeFork/1.0", "Accept": "image/png"},
        )
        try:
...
# Alias retained for clarity and compatibility.
...
class LLMBackend:
    ...
    def _pollinations_chat(self, model: str, messages: list[dict]) -> str:
        ...
        for url in urls:
            req = urllib.request.Request(
                url,
                headers={
                    "User-Agent": "Mozilla/5.0 ACKondosDiffusionBeeFork/1.0",
                    "Accept": "text/plain",
    ...
    def _openai_chat(self, provider, model, messages, temperature) -> str:
        ...
        if provider == "openrouter":
            headers["HTTP-Referer"] = "https://localhost/ac-kondos-diffusion-bee-fork"
...
# ---------------------------------------------------------------------------
# Job engine (AC Kondo's Diffusion Bee Fork async queue)
# ---------------------------------------------------------------------------
...
class ACKondosDiffusionBeeEngine:
    """Central backend for BFL FLUX protocols, LLMs, and history."""
    ...
    def _run(self, job: GenJob) -> None:
        ...
        stem = f"{prefix}_{job.id}" if kind == "video" else ac_kondos_diffusion_bee_image_stem(full_prompt)
...
class LLMDownloadWindow(tk.Toplevel):
    def __init__(self, app: "App"):
        ...
        tk.Label(
            head,
            text=f"Pre-baked Hugging Face GGUFs for FLUX vibes · download into {LLM_DIR} · vibe-add any repo",
        ...
        tk.Button(
            ...
            activebackground=BUTTON_BG_ACTIVE, activeforeground=BUTTON_TEXT_ACTIVE,
        ...
        tk.Label(
            right,
            text=f"Weights stay local. Point Ollama / LM Studio / llama.cpp at {LLM_DIR} after download.",
...
class ToolWorkspace(tk.Toplevel):
    def __init__(self, app: "App", title: str, mode: str):
        ...
        tk.Label(
            head,
            text=f"{APP_NAME} · local FLUX engine · offline when keyed",
        ...
        # Two-column applet layout: controls max-width 350 + thin border
        left = tk.Frame(
        ...
        # Advanced options (steps / CFG / batch)
        ...
        self.gen_btn = tk.Button(
            ...
            command=self._generate, bg=BUTTON_BG, fg=BUTTON_TEXT,
            activebackground=BUTTON_BG_ACTIVE, activeforeground=BUTTON_TEXT_ACTIVE,
        ...
        # Two-column right workspace and image engine stage
        right = tk.Frame(
            body, bg=ENGINE,
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Compact desktop default ~770×550
        ...
        if self.cfg.hf_active_llm:
            for item in self.hf_store.catalog():
                if item.get("name") == self.cfg.hf_active_llm:
                    ...
                    break
        self.engine = ACKondosDiffusionBeeEngine(self.cfg, self.history, self.hf_store)
    ...
    def _show_splash(self):
        """Display the centered AC Kondo splash screen."""
        splash = tk.Frame(self, bg=CONTENT)
        splash.place(x=0, y=0, relwidth=1, relheight=1)
        wrap = tk.Frame(splash, bg=CONTENT)
        wrap.place(relx=0.5, rely=0.5, anchor="center")
        tk.Label(
            wrap, text="AC Kondo's Diffusion Bee Fork", bg=CONTENT, fg=TEXT,
            font=(UI_FONT, 30, "bold"),
        ).pack()
        tk.Label(
            wrap, text="Fork 1.0", bg=CONTENT, fg=MUTED,
            font=(UI_FONT, 18),
        ).pack(pady=(4, 0))
        tk.Label(
            wrap, text="AC Kondo build · 100% local when keyed",
            bg=CONTENT, fg=MUTED,
            font=(UI_FONT, 12),
        ).pack(pady=(18, 0))
    ...
    def _build_shell(self):
        ...
        collapse = tk.Label(
            drag, text="☰", bg=BUTTON_BG, fg=BUTTON_TEXT, font=(UI_FONT, 14),
            cursor="hand2",
        )
        collapse.pack(side="right", padx=12, pady=15)
        collapse.bind("<Button-1>", lambda e: self._toggle_sidebar())
        collapse.bind("<Enter>", lambda e: collapse.configure(bg=BUTTON_BG_ACTIVE, fg=BUTTON_TEXT_ACTIVE))
        collapse.bind("<Leave>", lambda e: collapse.configure(bg=BUTTON_BG, fg=BUTTON_TEXT))
        ...
        self.theme_btn = tk.Label(
            ...
            bg=BUTTON_BG, fg=BUTTON_TEXT, font=(UI_FONT, 12, "bold"),
        ...
        self.theme_btn.bind(
            "<Enter>",
            lambda e: self.theme_btn.configure(bg=BUTTON_BG_ACTIVE, fg=BUTTON_TEXT_ACTIVE),
        )
        self.theme_btn.bind(
            "<Leave>",
            lambda e: self.theme_btn.configure(bg=BUTTON_BG, fg=BUTTON_TEXT),
        )
        ...
        menu_btn = tk.Label(
            tools, text="⋮", bg=BUTTON_BG, fg=BUTTON_TEXT, font=(UI_FONT, 18),
            cursor="hand2", padx=10,
        )
        menu_btn.pack(side="left")
        menu_btn.bind("<Button-1>", lambda e: self._open_overflow_menu())
        menu_btn.bind("<Enter>", lambda e: menu_btn.configure(bg=BUTTON_BG_ACTIVE, fg=BUTTON_TEXT_ACTIVE))
        menu_btn.bind("<Leave>", lambda e: menu_btn.configure(bg=BUTTON_BG, fg=BUTTON_TEXT))
    ...
    def _select_tab(self, key):
        # Generation tools → open the fork's applet workspace
    ...
    def _build_activity_page(self, parent):
        ...
        tk.Label(header, text=f"Live job queue from {APP_NAME}.",
    ...
    def _build_settings_page(self, parent):
        ...
        tk.Label(
            header, text=f"API keys & defaults. Saved to {CONFIG_PATH}",
        ...
        tk.Label(
            form,
            text="Output format: PNG (JPEG off for all FLUX models)",
    ...
    def _make_card(self, grid, row, col, title, desc, kind, mode):
        """Build a 280×230 tool card with preview, description, and Open action."""
        outer = tk.Frame(
        ...
        btn = tk.Label(
            bottom, text="Open", bg=BUTTON_BG, fg=BUTTON_TEXT,
        ...
        btn.bind("<Enter>", lambda e, b=btn: b.configure(bg=BUTTON_BG_ACTIVE, fg=BUTTON_TEXT_ACTIVE))
        btn.bind("<Leave>", lambda e, b=btn: b.configure(bg=BUTTON_BG, fg=BUTTON_TEXT))
